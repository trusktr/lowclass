...
type ReplaceCtorReturn<T, TReturn> = T extends new (...a: infer A) => unknown
    ? new (...a: A) => TReturn
    : never
...
type PrivateHelper = <T>(
    self: T
) => T extends { __: { private: infer TPrivate } } ? TPrivate : never
type PublicHelper = <T>(self: T) => Omit<T, ImplementationKeys> // TODO validate instance is public?
type ProtectedHelper = <T>(
    self: T
) => T extends { __: { protected: infer TProtected } } ? TProtected : never
...
export function Class(
    ...
    extends<TBase extends Constructor, T>(
        ...
        ? FunctionToConstructor<
              ConstructorOrDefault<T>,
              Id<InstanceType<TBase> & OmitImplementationKeys<T>>
          > &
              Id<StaticsAndProtected<T> & Pick<TBase, keyof TBase>>
        : ReplaceCtorReturn<TBase, Id<InstanceType<TBase>>> &
              Id<StaticsAndProtected<T> & Pick<TBase, keyof TBase>>
}
export function Class<T>(
    ...
    brand?: object
): FunctionToConstructor<ConstructorOrDefault<T>, Id<OmitImplementationKeys<T>>> &
    Id<StaticsAndProtected<T>>
export function Class<T>(
    name: string,
