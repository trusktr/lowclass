...
function runTests() {
    ...
    log('testing Proxy version...')
    const start1 = performance.now()
    for (let i = 0; i < 10000; i++) testProxySpeed()
    ...
    log('testing class-factory-mixin version...')
    const start2 = performance.now()
    for (let i = 0; i < 10000; i++) testMixinSpeed()
    const end2 = performance.now()
    const totalTime2 = end2 - start2
    log(`...done. Total time class-factory-mixin version: ${totalTime2}ms`)

    log(
        `${totalTime1 < totalTime2 ? 'Proxy version' : 'class-factory-mixin'} version was faster than ${
            totalTime1 < totalTime2 ? 'class-factory-mixin' : 'Proxy'
        } version by a factor of ${totalTime1 < totalTime2 ? totalTime2 / totalTime1 : totalTime1 / totalTime2}`,
    )
}

function hasProto(obj: Object, proto: Object): boolean {
    let current = (obj as any).__proto__
    while ((current = current.__proto__)) if (current === proto) return true
    return false
}
